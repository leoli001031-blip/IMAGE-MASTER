...
assert.match(
  ...
  /const outputSourceVersion = getOutputPreviewSourceVersion\(item\.metadata \?\? \{\}\)[\s\S]*useState\(Boolean\(item\.compareWithSource\)\)[\s\S]*setCompareWithSource\(Boolean\(item\.compareWithSource\)\)[\s\S]*OutputPreviewComparisonImage[\s\S]*label="上一版"[\s\S]*label="当前版"[\s\S]*上一版：\{outputSourceVersion\.title\}[\s\S]*退出对比[\s\S]*对比当前[\s\S]*function getOutputPreviewSourceVersion[\s\S]*rerunSourceArtifactUrl[\s\S]*rerunSourceResultUrl[\s\S]*revisionSource \?\? \{\}, "url"/,
  "canvas image preview should show, compare, and optionally default to previous-version comparison for rerun and Agent revision images"
);
