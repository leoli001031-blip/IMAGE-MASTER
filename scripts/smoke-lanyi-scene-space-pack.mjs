...
const sceneId = "lijiang-snow-mountain-space-pack";

fs.mkdirSync(outDir, { recursive: true });

const manifestPath = args.manifest || findLatestPanoramaManifest(outDir);
if (!manifestPath) {
  throw new Error("Missing panorama manifest. Run smoke:scene-panorama-perspective first or pass --manifest.");
}

const sourceManifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
const panoramaPath = sourceManifest.panorama?.imagePath;
if (!panoramaPath || !fs.existsSync(panoramaPath)) {
  throw new Error(`Manifest does not point to a readable panorama image: ${manifestPath}`);
}

const sceneJson = buildSceneJson();
const sceneJsonPath = path.join(outDir, `scene-space-pack-${sceneId}-${stamp}.json`);
fs.writeFileSync(sceneJsonPath, JSON.stringify(sceneJson, null, 2));

const spatialMapPath = path.join(outDir, `scene-space-pack-${sceneId}-${stamp}-spatial-map.png`);
await renderSpatialMap(spatialMapPath);

const cubemapFaces = await projectCubemapFaces({
  panoramaPath,
  outDir,
  prefix: `scene-space-pack-${sceneId}-${stamp}`,
});

const boardPath = path.join(outDir, `scene-space-pack-board-${sceneId}-${stamp}.png`);
await composeSpacePackBoard({
  panoramaPath,
  spatialMapPath,
  cubemapFaces,
  boardPath,
...
if (!args["no-generate"]) {
  ...
  finalGeneration = await generateFinalPerspective({
    face,
    spatialMapPath,
    sceneJson,
  });
...
const outputManifestPath = path.join(outDir, `scene-space-pack-${sceneId}-${stamp}.manifest.json`);
...
async function generateFinalPerspective({ face, spatialMapPath, sceneJson }) {
  ...
  const prompt = [
    "Create one clean normal-perspective commercial scene plate from this scene space pack.",
    `Selected face: ${face.id.toUpperCase()} - ${face.description}.`,
    "Input image 1 is the selected cubemap face extracted from the panorama.",
  ...
  const prefix = `scene-space-pack-${sceneId}-final-${face.id}-${stamp}`;
  ...
  const clientRequestId = `image2.scene_space_pack.${face.id}.${crypto.randomUUID()}`;
  fs.writeFileSync(promptPath, prompt);

  const faceBuffer = fs.readFileSync(face.imagePath);
  const spatialMapBuffer = fs.readFileSync(spatialMapPath);
  console.log(`[final-${face.id}] started size=${perspectiveSize}`);
      ...
      input: [
        {
          ...
          content: [
            ...
            {
              type: "input_image",
              image_url: `data:${inferMime(face.imagePath)};base64,${faceBuffer.toString("base64")}`,
            },
            {
              type: "input_image",
              image_url: `data:image/png;base64,${spatialMapBuffer.toString("base64")}`,
            },
          ],
        },
      ],
      tools: [{ type: "image_generation", size: perspectiveSize, quality }],
      stream: true,
    }),
  });
  ...
  while (reader) {
    ...
    if (!firstLogged) {
      firstLogged = true;
      console.log(`[final-${face.id}] first chunk ${elapsed}ms`);
    }
  }
  ...
  const result = {
    ...
    referenceFacePath: face.imagePath,
    spatialMapPath,
    firstChunkMs,
    lastChunkMs,
  ...
  console.log(`[final-${face.id}] done image=${result.imageReturned} status=${result.status} total=${result.totalMs}ms`);
  return result;
}
...
async function projectCubemapFaces({ panoramaPath, outDir, prefix }) {
  const faces = [
    ...
    { id: "back", yaw: 180, pitch: 0, description: "terrace wall, entrance, and walking zone" },
    { id: "left", yaw: -90, pitch: 0, description: "pine, rough stone, and textile side" },
    { id: "up", yaw: 0, pitch: -90, description: "sky, roof edge, and upper light direction" },
...
async function renderSpatialMap(outputPath) {
  const sharp = (await import("sharp")).default;
  await sharp(Buffer.from(createSpatialMapSvg(900, 700))).png().toFile(outputPath);
}

async function composeSpacePackBoard({ panoramaPath, spatialMapPath, cubemapFaces, boardPath }) {
  const sharp = (await import("sharp")).default;
  const width = 1800;
  const gutter = 16;
  const panoramaHeight = 720;
  const bottomHeight = 700;
  const faceSize = Math.floor((width - 900 - gutter * 5) / 3);
  ...
  const composites = [
    ...
    },
    {
      input: await sharp(spatialMapPath)
        .resize({ width: 900, height: bottomHeight, fit: "contain", background })
        .png()
        .toBuffer(),
      left: gutter,
  ...
  for (let index = 0; index < Math.min(6, cubemapFaces.length); index += 1) {
  ...
  await sharp({
    create: {
      width,
      height: panoramaHeight + gutter + bottomHeight,
      channels: 4,
      background,
    },
  })
...
function buildSceneJson() {
  return {
    ...
    sceneId,
    confidence: "ai_pseudo_panorama_with_structured_spatial_constraints",
    coordinateSystem: {
      front: "north",
      right: "east",
      back: "south",
    ...
    anchors: {
      ...
      south_back: ["terrace wall", "entrance or walking zone", "open stone floor"],
    ...
    generationRules: [
      "Use the scene JSON and spatial map for positions.",
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="36" y="88" font-family="Inter, Arial, sans-serif" font-size="16" fill="#776856">Deterministic spatial constraint paired with panorama and cubemap faces.</text>
  ...
  <text x="${width / 2}" y="178" font-family="Inter, Arial, sans-serif" font-size="15" text-anchor="middle" fill="#6f6a62">low stone terrace wall</text>
  ...
  <text x="${width - 237}" y="${height / 2 - 8}" font-family="Inter, Arial, sans-serif" font-size="18" font-weight="700" text-anchor="middle" fill="#fff">architecture</text>
  ...
  <text x="220" y="${height - 292}" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="700" text-anchor="middle" fill="#314323">pine / stone</text>
...
function findLatestPanoramaManifest(directory) {
  if (!fs.existsSync(directory)) return null;
  return fs
    .readdirSync(directory)
    .filter((name) => /^scene-panorama-perspective-.*\.manifest\.json$/.test(name))
